...
- The **power set** of a given set `$A$` is denoted as `$\mathcal{P}(A)$` and constitutes all combinations of subsets of `$A$` (which also includes both `$A$` itself and `$\varnothing$`, based on the above definitions). Hence, if `$A = \{1, 2\}$`, then `$\mathcal{P}(A) = \{\{1\}, \{2\}, \{1, 2\}\}$`. It is called a power set because, if `$A$` is a finite set, then `$\mathcal{P}(A) = 2^{|A|}$`. Notice that there is no specification stating that `$A$` must be non-empty: `$\mathcal{P}(\{\varnothing\}) = 2^{|\{\varnothing\}|} = 2^0 = 1$`.
